...
def determine_output_directory(
    ...
    if context.wpt_context and context.wpt_context.test_contents:
        dirs = [
            Path(p).parent for p in context.wpt_context.test_contents.keys()
        ]
        if dirs:
            ...
            ui.success(
                f"Inferred output directory from existing tests: {rel_path}"
            )
    ...
    ui.info(
        f"Using target directory: {Path(validated_path).relative_to(wpt_path)}"
    )

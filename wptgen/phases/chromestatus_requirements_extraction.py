...
async def run_chromestatus_requirements_extraction(
  ...
  ui.on_phase_start(2, 'ChromeStatus Requirements Extraction')
  ...
  if not requirements_xml:
    extraction_prompt = jinja_env.get_template(
      'chromestatus_templates/requirements_extraction.jinja'
    ).render(
    ...
    extraction_system_prompt = jinja_env.get_template(
      'chromestatus_templates/requirements_extraction_system.jinja'
    ).render()
...
async def run_chromestatus_requirements_extraction_categorized(
  context: WorkflowContext,
  config: Config,
  llm: LLMClient,
  ui: UIProvider,
  ...
  ui.on_phase_start(2, 'ChromeStatus Requirements Extraction (Categorized)')
  ...
  if not requirements_xml:
    ...
    for category_name, category_description in REQUIREMENT_CATEGORIES:
      extraction_prompt = jinja_env.get_template(
        'chromestatus_templates/requirements_extraction_categorized.jinja'
      ).render(
      ...
      extraction_system_prompt = jinja_env.get_template(
        'chromestatus_templates/requirements_extraction_categorized_system.jinja'
      ).render(
...
async def run_chromestatus_requirements_extraction_iterative(
  context: WorkflowContext,
  config: Config,
  llm: LLMClient,
  ui: UIProvider,
  ...
  ui.on_phase_start(2, 'ChromeStatus Requirements Extraction (Iterative)')
  ...
  if not requirements_xml:
    ...
    while iteration <= max_iterations:
      ...
      extraction_prompt = jinja_env.get_template(
        'chromestatus_templates/requirements_extraction_iterative.jinja'
      ).render(
        feature_name=context.metadata.name,
        feature_description=context.metadata.description,
        specs=context.spec_contents,
      ...
      extraction_system_prompt = jinja_env.get_template(
        'chromestatus_templates/requirements_extraction_iterative_system.jinja'
      ).render()

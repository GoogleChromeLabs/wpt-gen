...
async def _fetch_explainer_contents(context: WorkflowContext,
                                    ui: UIProvider) -> None:
    """
    Fetches explainer contents if they are not already present in the context.
    """
    if (not context.explainer_contents and context.metadata and
            context.metadata.explainer_links):
        ...
        with ui.status('Fetching and extracting text from explainers...'):
            ...
            for url in context.metadata.explainer_links:
                if url not in context.explainer_contents:
                    ui.warning(
                        f'Failed to fetch or extract content from explainer: {url}'  # pylint: disable=line-too-long
                    )
...
async def run_requirements_extraction(
    ...
    if not requirements_xml:
        ...
        extraction_system_prompt = jinja_env.get_template(
            'requirements_extraction_system.jinja').render(
...
async def run_requirements_extraction_categorized(
    ...
    if not requirements_xml:
        ...
        for category_name, category_description in REQUIREMENT_CATEGORIES:
            extraction_prompt = jinja_env.get_template(
                'requirements_extraction_categorized.jinja').render(
            ...
            extraction_system_prompt = jinja_env.get_template(
                'requirements_extraction_categorized_system.jinja').render(
        ...
        ui.info(
            f'Launching {len(REQUIREMENT_CATEGORIES)} parallel extraction requests...'  # pylint: disable=line-too-long
        )
        total_tasks = len(REQUIREMENT_CATEGORIES)
        completed_count = 0

        async def wrap_with_progress(
                name: str, extraction_prompt: str,
                extraction_system_prompt: str) -> str | None:
            nonlocal completed_count
            res = await extract_for_category(name, extraction_prompt,
        ...
        for (name, _), response in zip(REQUIREMENT_CATEGORIES,
                                       responses,
                                       strict=True):
            if not response:
                continue
            ...
            new_reqs = re.findall(r'(<requirement\b[^>]*>.*?</requirement>)',
                                  response, re.DOTALL)

            if not new_reqs:
                # If no requirements, look for a rationale.
                rationale_match = re.search(r'<rationale>(.*?)</rationale>',
                                            response, re.DOTALL)
                if rationale_match:
                    ui.info(
                        f'No requirements found for category [{name}] {rationale_match.group(1).strip()}'  # pylint: disable=line-too-long
                    )
                continue
        ...
        requirements_xml = ('<requirements_list>\n  ' +
...
async def run_requirements_extraction_iterative(
    ...
    if not requirements_xml:
        ...
        while iteration <= max_iterations:
            ...
            extraction_prompt = jinja_env.get_template(
                'requirements_extraction_iterative.jinja').render(
                    feature_name=context.metadata.name,
                    feature_description=context.metadata.description,
                    specs=context.spec_contents,
                    mdn_contents=context.mdn_contents,
            ...
            extraction_system_prompt = jinja_env.get_template(
                'requirements_extraction_iterative_system.jinja').render(
                    has_mdn=bool(context.mdn_contents),
                    has_explainer=bool(context.explainer_contents),
                )
            ...
            if '<status>EXHAUSTED</status>' in response:
                ui.success(
                    f'Extraction complete: LLM signaled exhaustion at iteration {iteration}.'  # pylint: disable=line-too-long
                )
                break

...
async def run_context_assembly(web_feature_id: str, config: Config,
                               ui: UIProvider) -> WorkflowContext | None:
    ui.on_phase_start(1, 'Context Assembly')

    if config.chromestatus:
        metadata = await asyncio.to_thread(fetch_chromestatus_metadata,
                                           web_feature_id)
        if not metadata:
        ...
        if not feature_data:
            if config.spec_urls and config.feature_description:
                ui.warning(
                    f'Feature {web_feature_id} not found in the web-features repository.'  # pylint: disable=line-too-long
                )
                ...
                ui.print(
                    'To generate tests for an unregistered feature, please provide both a spec URL using --spec-urls '  # pylint: disable=line-too-long
            ...
            if extracted_wpt_urls:
                try:
                    ...
                    for invalid in invalid_paths:
                        ui.warning(
                            f'Referenced WPT test file could not be found or read: {invalid}'  # pylint: disable=line-too-long
                        )
    ...
    if config.include_mdn_docs:
        ...
        if mdn_urls:
            with ui.status(f'Fetching {len(mdn_urls)} MDN pages...'):
                # Fetch all MDN pages asynchronously using to_thread for the
                # synchronous fetch_and_extract_text
                results = await asyncio.gather(*[
                    asyncio.to_thread(fetch_and_extract_text, url)
                    for url in mdn_urls
                ])

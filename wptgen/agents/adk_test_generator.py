...
async def generate_test_with_adk(
    ...
    def report_generation_complete(file_paths: list[str]) -> dict[str, Any]:
        """Call this tool ONLY when you have successfully written all necessary test files to disk.  # pylint: disable=line-too-long

        Args:
            file_paths: A list of the absolute or relative file paths you generated.  # pylint: disable=line-too-long
    ...
    tools: list[Any] = list(
        create_agent_tools(wpt_root, ui, config.run_on_browser,
    ...
    instruction = system_template.render(test_type=test_type_enum.value,)

    # Prevent ADK's internal template parser from crashing when it encounters
    # WPT syntax like `{{host}}` or `{{variable}}` by mapping them to
    # themselves.
...
# Ensure the agent name is a valid Python identifier to avoid validation errors.
    ...
    feature_description = (context.metadata.description
                           if context.metadata else 'Unknown')
    ...
    try:
        events = runner.run_async(session_id=session.id,
                                  user_id='cli_user',
                                  new_message=content)

        # We just consume the stream to let the agent run.
        with ADKStreamManager(
                ui, include_thoughts=config.include_thoughts) as stream_manager:
...
# If the agent correctly called the completion tool, we read those files back
        for path_str in generated_paths:
            try:
                ...
                ui.error(
                    f"Failed to read securely generated file '{path_str}': {e}")

        return results

    finally:
        await runner.close()  # type: ignore[no-untyped-call]
        await session_service.delete_session(app_name='wpt-gen',
                                             user_id='cli_user',

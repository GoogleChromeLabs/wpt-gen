...
async def generate_test_with_adk(
    ...
    tools: list[Any] = list(
        create_agent_tools(
            wpt_root, ui, config.run_on_browser, config.run_on_channel
        )
    )
    ...
    feature_description = (
        context.metadata.description if context.metadata else "Unknown"
    )
    ...
    try:
        ...
        with ADKStreamManager(
            ui, include_thoughts=config.include_thoughts
        ) as stream_manager:

...
class RichUIProvider:
    ...
    def report_test_generated(
        ...
        if success:
            if fallback:
                self.console.print(  # pylint: disable=line-too-long
                    f'[green]✔ Saved (fallback):[/green] {path.absolute() if path else ""}'  # pylint: disable=line-too-long,inconsistent-quotes
                )
            else:
                self.console.print(

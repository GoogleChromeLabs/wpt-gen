...
async def test_run_coverage_audit_token_limit_exceeded(
    ...
    context = WorkflowContext(
        feature_id='test',
        requirements_xml=
        '<requirements><requirement id="R1">Test</requirement></requirements>',  # pylint: disable=line-too-long
        wpt_context=WPTContext(),
    )

    mock_llm.prompt_exceeds_input_token_limit.return_value = True

    result = await run_coverage_audit(context, mock_config, mock_llm, mock_ui,
                                      mock_jinja_env)
...
async def test_provide_coverage_report_save_error(
    ...
    context = WorkflowContext(feature_id='test',
                              audit_response='Mock audit response')
...
@pytest.mark.asyncio
async def test_provide_coverage_report_save_success(mock_config: Config,
                                                    mock_ui: MagicMock,
                                                    tmp_path: Path) -> None:
    mock_config.output_dir = str(tmp_path)
    context = WorkflowContext(feature_id='test',
...
def test_partition_requirements_xml_empty() -> None:  # pylint: disable=inconsistent-quotes
    assert not partition_requirements_xml('')  # pylint: disable=inconsistent-quotes
...
async def test_run_coverage_audit_always_brief_suggestions(
    ...
    await run_coverage_audit(context, mock_config, mock_llm, mock_ui,
                             mock_jinja_env)

    system_template_mock.render.assert_called_once_with(brief_suggestions=True,
                                                        spec_urls=[])

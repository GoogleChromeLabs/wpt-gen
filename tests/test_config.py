...
def test_load_config_spec_urls(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = load_config(config_path='non_existent_dummy.yaml',
                         spec_urls_override=spec_urls)

    assert config.spec_urls == spec_urls


def test_load_config_output_dir(monkeypatch: pytest.MonkeyPatch,
                                tmp_path: Path) -> None:
...
def test_validate_output_dir_permission_error(tmp_path: Path) -> None:
    ...
    try:
        with pytest.raises(ValueError,
                           match='CRITICAL: Cannot write to output directory'):
...
def test_load_config_timeout_minimum(monkeypatch: pytest.MonkeyPatch,
    ...
    assert ('Requested timeout 1s is less than the minimum allowed'
            in caplog.text)
...
def test_load_config_max_parallel_requests(
    ...
    config = load_config(config_path='non_existent_dummy.yaml',
                         max_parallel_requests_override=20)
...
def test_load_config_yes_tests(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Case 2: Override to True
    config = load_config(config_path='non_existent_dummy.yaml',
                         yes_tests_override=True)
    assert config.yes_tests is True


def test_load_config_loaded_from(monkeypatch: pytest.MonkeyPatch,
                                 tmp_path: Path) -> None:
...
def test_load_config_deep_merges_phase_mapping(monkeypatch: pytest.MonkeyPatch,
                                               tmp_path: Path) -> None:
    """Test that setting a single nested property preserves sibling defaults."""
    monkeypatch.setenv('GEMINI_API_KEY', 'mock-key')

    config_file = tmp_path / 'wpt-gen.yml'  # pylint: disable=line-too-long
...
def test_load_config_deep_merges_categories(monkeypatch: pytest.MonkeyPatch,

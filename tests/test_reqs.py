...
async def test_run_requirements_extraction_no_cache(
    ...
    result = await run_requirements_extraction(base_context, mock_config,
                                               mock_llm, mock_ui, jinja_env,
                                               tmp_path)
...
async def test_run_requirements_extraction_fails(
    ...
    mocker.patch('wptgen.phases.requirements_extraction.confirm_prompts')
    mocker.patch('wptgen.phases.requirements_extraction.generate_safe',
                 return_value=None)
...
async def test_run_requirements_extraction_iterative_fails(
    ...
    mocker.patch('wptgen.phases.requirements_extraction.confirm_prompts')
    mocker.patch('wptgen.phases.requirements_extraction.generate_safe',
                 return_value=None)  # Fails initial
...
async def test_run_requirements_extraction_cache_success(
    ...
    result = await run_requirements_extraction(base_context,
                                               mock_config, mock_llm, mock_ui,
                                               MagicMock(), tmp_path)
    assert result is not None
...
async def test_run_requirements_extraction_iterative_rationale(
    ...
    result = await run_requirements_extraction_iterative(
        base_context, mock_config, mock_llm, mock_ui, jinja_env, tmp_path)  # pylint: disable=line-too-long

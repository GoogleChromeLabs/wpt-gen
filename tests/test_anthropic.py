...
def test_get_llm_client_anthropic(mocker: MockerFixture, anthropic_config: Config) -> None:
...
def test_anthropic_generate_content(mocker: MockerFixture, anthropic_config: Config) -> None:
  ...
  client = AnthropicClient(api_key=anthropic_config.api_key, model=anthropic_config.default_model)
  ...
  mock_anthropic_class.assert_called_once_with(api_key='mock-anthropic-key', timeout=600.0)
...
def test_anthropic_count_tokens(mocker: MockerFixture, anthropic_config: Config) -> None:
  ...
  mock_instance.messages.count_tokens.assert_called_once_with(
    model='claude-3-7-sonnet-20250219', messages=[{'role': 'user', 'content': 'Hello world'}]
  )


def test_anthropic_retry_on_failure(mocker: MockerFixture, anthropic_config: Config) -> None:
...
def test_anthropic_prompt_exceeds_limit(mocker: MockerFixture, anthropic_config: Config) -> None:

...
def _create_mock_config(provider: str, api_key: str, default_model: str,
                        wpt_path: Path | str) -> Config:
    return Config(
        provider=provider,
...
def test_setup_adk_environment_google(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = _create_mock_config('google', 'fake-key', 'gemini-3.1-pro-preview',
                                 '/tmp')
...
def test_setup_adk_environment_anthropic(
    ...
    monkeypatch.setattr(os, 'environ', {})
    config = _create_mock_config('anthropic', 'fake-key', 'claude-opus-4-6',
                                 '/tmp')
    model = setup_adk_environment(config)
...
def test_validate_safe_path_traversal(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError,
                       match='is outside the designated WPT repository root'):
...
def test_file_tools_read_file(tmp_path: Path) -> None:
    ...
    tools = create_agent_tools(wpt_root, MagicMock(spec=UIProvider), 'chrome',
                               'canary')
...
def test_agent_tools_run_wpt_test(tmp_path: Path, mocker: Any) -> None:
    ...
    mock_run.side_effect = subprocess.TimeoutExpired(cmd='run',
                                                     timeout=60,
...
def test_fetch_spec_content(tmp_path: Path, mocker: Any) -> None:
    ...
    mocker.patch('wptgen.agents.tools.fetch_and_extract_text',

...
def test_report_coverage_audit(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
...
def test_report_audit_worksheet(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
...
def test_report_test_suggestion(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
...
def test_report_context_summary(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
...
def test_report_token_usage_auto_confirm(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
    ui.report_token_usage(
        "Phase", "Model", [(100, False, "Task")], 100, auto_confirmed=True
    )
    assert mock_console.print.call_count == 2


def test_report_audit_worksheet_covered(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
...
def test_report_generation_start(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
...
def test_report_test_generated_success(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
...
def test_report_test_generated_success_fallback(
    ...
    ui.report_test_generated(
        "test", True, Path("/tmp/test.html"), fallback=True
    )
    mock_console.print.assert_called_once()


def test_report_test_generated_fail(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
...
def test_report_generation_summary_empty(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
...
def test_progress_indicator_no_outstanding(
    mocker: MockerFixture, ui: RichUIProvider
) -> None:
    mock_progress_class = mocker.patch("wptgen.ui.Progress")
    mock_progress = mock_progress_class.return_value.__enter__.return_value
    mock_progress.add_task.return_value = "task_1"
    ...
    mock_progress.update.assert_called_once_with(
        "task_1", description="Updated"
    )


def test_report_token_usage_multiple(
    ui: RichUIProvider, mock_console: MagicMock
) -> None:
    ui.report_token_usage(
        "Phase", "Model", [(100, False, "Task"), (200, False, "Task2")], 300
    )
    assert mock_console.print.call_count == 2

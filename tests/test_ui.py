...
def test_success(ui: RichUIProvider, mock_console: MagicMock) -> None:
    ...
    mock_console.print.assert_called_once_with(
        '[bold green]✔[/bold green] success message')
...
def test_report_token_usage(ui: RichUIProvider,
                            mock_console: MagicMock) -> None:
    """Test report_token_usage semantic method."""
    ui.report_token_usage('Phase', 'Model', [(100, False, 'Task')], 100)
    mock_console.print.assert_called_once()


def test_report_llm_response_js_suffix(mocker: MockerFixture,
                                       ui: RichUIProvider,
                                       mock_console: MagicMock) -> None:
    ...
    args, kwargs = mock_syntax.call_args  # pylint: disable=unused-variable
    assert args[0] == response
    assert args[1] == 'javascript'
    mock_console.print.assert_called_once()


def test_report_llm_response_html_suffix(mocker: MockerFixture,
                                         ui: RichUIProvider,
                                         mock_console: MagicMock) -> None:
...
def test_report_llm_response_fallback_gen(mocker: MockerFixture,
                                          ui: RichUIProvider,
                                          mock_console: MagicMock) -> None:
...
def test_report_llm_response_fallback_eval(mocker: MockerFixture,
                                           ui: RichUIProvider,
                                           mock_console: MagicMock) -> None:
...
def test_report_llm_response_fallback_default(mocker: MockerFixture,
                                              ui: RichUIProvider,
                                              mock_console: MagicMock) -> None:
...
def test_report_coverage_audit(ui: RichUIProvider,
                               mock_console: MagicMock) -> None:
...
def test_report_audit_worksheet(ui: RichUIProvider,
                                mock_console: MagicMock) -> None:
...
def test_report_test_suggestion(ui: RichUIProvider,
                                mock_console: MagicMock) -> None:
...
def test_report_generation_summary(ui: RichUIProvider,
                                   mock_console: MagicMock) -> None:
...
def test_progress_indicator(mocker: MockerFixture, ui: RichUIProvider) -> None:
    ...
    mock_progress.update.assert_called_once_with(
        'task_1', description='Updated (8 outstanding)')
...
def test_print_diff(ui: RichUIProvider, mock_console: MagicMock,
...
def test_print_diff_empty(ui: RichUIProvider, mock_console: MagicMock,
                          mocker: MockerFixture) -> None:
    mock_syntax = mocker.patch('wptgen.ui.Syntax')
    ui.print_diff('same\n', 'same\n', 'file.txt')
...
def test_on_phase_complete(ui: RichUIProvider, mock_console: MagicMock) -> None:
    ...
    mock_console.print.assert_called_once_with(
        '[bold green]✔[/bold green] Phase 1 complete.')


def test_report_context_summary(ui: RichUIProvider,
                                mock_console: MagicMock) -> None:
    ui.report_context_summary(1, 2, 3, 4)
    mock_console.print.assert_called_once()


def test_report_token_usage_warnings(ui: RichUIProvider,
                                     mock_console: MagicMock) -> None:
    ui.report_token_usage('Phase', 'Model', [(100, True, 'Task')], 100)
    assert mock_console.print.call_count == 2


def test_report_token_usage_auto_confirm(ui: RichUIProvider,
                                         mock_console: MagicMock) -> None:
    ui.report_token_usage('Phase',
                          'Model', [(100, False, 'Task')],
                          100,
                          auto_confirmed=True)
    assert mock_console.print.call_count == 2


def test_report_audit_worksheet_covered(ui: RichUIProvider,
                                        mock_console: MagicMock) -> None:
    ui.report_audit_worksheet('R1: Req -> [COVERED by file.html]')
    assert mock_console.print.call_count == 2


def test_report_generation_start(ui: RichUIProvider,
                                 mock_console: MagicMock) -> None:
...
def test_report_test_generated_success(ui: RichUIProvider,
                                       mock_console: MagicMock) -> None:
...
def test_report_test_generated_success_fallback(
        ui: RichUIProvider, mock_console: MagicMock) -> None:
    ui.report_test_generated('test',
                             True,
                             Path('/tmp/test.html'),
                             fallback=True)
    mock_console.print.assert_called_once()
...
def test_report_test_generated_fail(ui: RichUIProvider,
                                    mock_console: MagicMock) -> None:
    ui.report_test_generated('test', False)
    mock_console.print.assert_called_once_with(
        '[bold red]✘[/bold red] Failed to generate: test')


def test_report_generation_summary_empty(ui: RichUIProvider,
                                         mock_console: MagicMock) -> None:
...
def test_progress_indicator_no_outstanding(mocker: MockerFixture,
                                           ui: RichUIProvider) -> None:
    mock_progress_class = mocker.patch('wptgen.ui.Progress')
    mock_progress = mock_progress_class.return_value.__enter__.return_value
    mock_progress.add_task.return_value = 'task_1'
    ...
    mock_progress.update.assert_called_once_with('task_1',
                                                 description='Updated')


def test_report_token_usage_multiple(ui: RichUIProvider,
                                     mock_console: MagicMock) -> None:
    ui.report_token_usage('Phase', 'Model', [(100, False, 'Task'),
                                             (200, False, 'Task2')], 300)
    assert mock_console.print.call_count == 2

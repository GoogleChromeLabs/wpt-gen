...
@pytest.mark.asyncio
async def test_run_context_assembly_success(mock_config: Config,
                                            mock_ui: MagicMock,
    ...
    mocker.patch('wptgen.phases.context_assembly.find_feature_tests',
                 return_value=[])
    mocker.patch(
...
@pytest.mark.asyncio
async def test_run_context_assembly_with_mdn(mock_config: Config,
                                             mock_ui: MagicMock,
                                             mocker: MockerFixture) -> None:
...
@pytest.mark.asyncio
async def test_run_context_assembly_without_mdn(mock_config: Config,
                                                mock_ui: MagicMock,
                                                mocker: MockerFixture) -> None:
...
async def test_run_context_assembly_unregistered_with_params(
        mock_config: Config, mock_ui: MagicMock, mocker: MockerFixture) -> None:
    ...
    mocker.patch('wptgen.phases.context_assembly.fetch_feature_yaml',
                 return_value=None)
    mocker.patch('wptgen.phases.context_assembly.fetch_mdn_urls',
                 return_value=[])
    mocker.patch(
        'wptgen.phases.context_assembly.fetch_and_extract_text',
        return_value='Spec Content',
...
@pytest.mark.asyncio
async def test_run_context_assembly_not_found(mock_config: Config,
                                              mock_ui: MagicMock,
                                              mocker: MockerFixture) -> None:
...
@pytest.mark.asyncio
async def test_run_context_assembly_no_specs(mock_config: Config,
                                             mock_ui: MagicMock,
                                             mocker: MockerFixture) -> None:
...
async def test_run_context_assembly_chromestatus_with_wpt_descr(
    ...
    metadata = FeatureMetadata('Feat',
                               'Desc', ['http://spec'],
                               wpt_descr='css/test.html')
    ...
    mock_validate.assert_called_once_with(['css/test.html'],
                                          mock_config.wpt_path)
    ...
    mock_gather.assert_called_once_with(['/abs/css/test.html'],
                                        mock_config.wpt_path)
...
async def test_run_context_assembly_chromestatus_too_many_tests(
    ...
    # Should also have warned that no tests were loaded (since
    # find_feature_tests returned [])
    mock_ui.warning.assert_any_call(
        'No existing Web Platform Tests were successfully loaded.')


@pytest.mark.asyncio
async def test_run_requirements_extraction_cached(mock_config: Config,
                                                  mock_ui: MagicMock,
                                                  mock_llm: MagicMock,
                                                  tmp_path: Path) -> None:
    ...
    res = await run_requirements_extraction(context, mock_config, mock_llm,
                                            mock_ui, MagicMock(), cache_dir)
...
async def test_run_requirements_extraction_with_explainer(
    ...
    with patch(
            'wptgen.phases.requirements_extraction.generate_safe',  # pylint: disable=line-too-long
            return_value=
        ...
        await run_requirements_extraction(context, mock_config, mock_llm,
                                          mock_ui, jinja_env, tmp_path)
...
async def test_run_requirements_extraction_fetches_explainer(
        mock_config: Config, mock_ui: MagicMock, mock_llm: MagicMock,
        tmp_path: Path) -> None:  # pylint: disable=line-too-long
    """Test that requirements extraction fetches explainer content if missing from context."""  # pylint: disable=line-too-long
    metadata = FeatureMetadata('Feat', 'Desc', ['http://spec'])
...
async def test_run_requirements_extraction_explainer_fetch_warning(
    ...
    mock_ui.warning.assert_called_with(  # pylint: disable=line-too-long
        'Failed to fetch or extract content from explainer: http://explainer-fail'  # pylint: disable=line-too-long
    )
...
async def test_run_requirements_extraction_categorized_with_explainer(
    ...
    with patch(
        ...
        await run_requirements_extraction_categorized(context, mock_config,
                                                      mock_llm, mock_ui,
                                                      jinja_env, tmp_path)
...
async def test_run_requirements_extraction_categorized(
    ...
    mocker.patch(
        'wptgen.phases.requirements_extraction.generate_safe',
        side_effect=[  # pylint: disable=line-too-long
            '<requirements_list><requirement id="R1"><category>Existence</category><description>D1</description></requirement></requirements_list>',  # pylint: disable=line-too-long
    ...
    res = await run_requirements_extraction_categorized(context, mock_config,
                                                        mock_llm, mock_ui,
                                                        jinja_env, tmp_path)
...
async def test_run_requirements_extraction_categorized_partial_empty(
    ...
    mocker.patch(
        ...
        side_effect=[  # pylint: disable=line-too-long
            ...
            '<requirements_list></requirements_list>',  # Empty  # pylint: disable=line-too-long
...
async def test_run_requirements_extraction_categorized_with_rationale(
    ...
    mock_ui.info.assert_any_call(  # pylint: disable=line-too-long
...
@pytest.mark.asyncio
async def test_provide_coverage_report(mock_config: Config, mock_ui: MagicMock,
                                       tmp_path: Path) -> None:
    """Test saving and displaying the coverage report."""
    context = WorkflowContext(feature_id='feat-id',
                              audit_response='Audit markdown')
...
@pytest.mark.asyncio
async def test_run_test_generation_satisfied(mock_config: Config,
                                             mock_ui: MagicMock,
                                             mock_llm: MagicMock) -> None:
    ...
    res = await run_test_generation(context, mock_config, mock_llm, mock_ui,
                                    jinja_env)
...
@pytest.mark.asyncio
async def test_run_test_generation_no_suggestions(mock_config: Config,
                                                  mock_ui: MagicMock,
                                                  mock_llm: MagicMock) -> None:
...
@pytest.mark.asyncio
async def test_run_test_generation_none_selected(mock_config: Config,
                                                 mock_ui: MagicMock,
                                                 mock_llm: MagicMock) -> None:
    """Test test generation when user rejects all suggestions."""  # pylint: disable=line-too-long
...
async def test_run_test_generation_adk(
        ...
        mock_ui: MagicMock,
        mock_llm: MagicMock,
        mocker: MockerFixture,  # pylint: disable=line-too-long
) -> None:
    """Test that ADK generation branches correctly and calls _generate_adk_loop."""  # pylint: disable=line-too-long
    from wptgen.models import WorkflowContext
    from wptgen.phases.generation import run_test_generation  # pylint: disable=line-too-long
    ...
    results = await run_test_generation(context, mock_config, mock_llm, mock_ui,
                                        jinja_env)
...
async def test_generate_adk_loop(
        mock_config: Config,
        mock_ui: MagicMock,  # pylint: disable=line-too-long
        mocker: MockerFixture) -> None:
    ...
    mock_config.brief_suggestions = False  # pylint: disable=line-too-long
    ...
    mock_adk.return_value = [(Path('test_dir/feat.html'), 'content',
                              suggestion_xml)]
    ...
    results = await _generate_adk_loop([suggestion_xml], context, mock_config,
                                       mock_ui, jinja_env)

...
def test_normalize_wpt_path() -> None:
    ...
    assert (normalize_wpt_path('path/to/test.any.worker.html') ==
            'path/to/test.any.js')
...
def test_validate_wpt_paths(tmp_path: Path) -> None:
    ...
    (test_dir / 'subdir' /
     'hidden.html').touch()  # Should be ignored (not top-level)
...
def test_fetch_and_extract_text_success(mocker: MockerFixture) -> None:  # pylint: disable=line-too-long
...
def test_resolve_patterns_basic_and_recursive(tmp_path: Path) -> None:  # pylint: disable=line-too-long
...
def test_resolve_patterns_negative_exclusion(tmp_path: Path) -> None:  # pylint: disable=line-too-long
...
def test_find_feature_tests_missing_directory() -> None:
    ...
    with pytest.raises(ValueError,
                       match='The directory provided does not exist'):
        find_feature_tests('/path/that/absolutely/does/not/exist', 'grid')


def test_find_feature_tests_malformed_yaml(tmp_path: Path) -> None:  # pylint: disable=line-too-long
...
def test_find_feature_tests_feature_not_found(tmp_path: Path) -> None:  # pylint: disable=line-too-long
    """Test that if a feature ID is not in any YAML, it returns an empty list."""  # pylint: disable=line-too-long
    (tmp_path / 'WEB_FEATURES.yml'
    ).write_text("features:\n  - name: grid\n    files:\n      - '*.html'")
...
def test_extract_dependencies() -> None:  # pylint: disable=line-too-long
...
def test_resolve_dependency_path(tmp_path: Path) -> None:  # pylint: disable=line-too-long
    ...
    # Absolute repo path
    resolved_abs = resolve_dependency_path(test_file,
    ...
    assert (resolve_dependency_path(test_file, 'http://example.com/js.js',
                                    wpt_root) is None)
...
def test_gather_local_test_context(tmp_path: Path) -> None:  # pylint: disable=line-too-long
...
def test_find_feature_tests_yaml_missing_features(tmp_path: Path) -> None:
    ...
    feat_dir.mkdir()
    (feat_dir / 'WEB_FEATURES.yml').write_text('something: else',
                                               encoding='utf-8')

    assert find_feature_tests(str(tmp_path), 'grid') == []


def test_find_feature_tests_exception(tmp_path: Path,
...
def test_resolve_dependency_path_invalid(tmp_path: Path) -> None:
    ...
    assert (resolve_dependency_path(test_file, '../../outside.js', wpt_root)
            is None)


def test_gather_local_test_context_exception(tmp_path: Path,
                                             mocker: MockerFixture) -> None:  # pylint: disable=line-too-long
...
def test_fetch_feature_yaml_draft(mocker: MockerFixture) -> None:
    ...
    request_obj = mock_urlopen.call_args[0][0]
    assert (
        request_obj.full_url ==  # pylint: disable=line-too-long

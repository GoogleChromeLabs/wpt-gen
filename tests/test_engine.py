...
@pytest.fixture
def engine(mock_config: Config, mock_llm: MagicMock,
           mock_ui: MagicMock) -> WPTGenEngine:
...
@pytest.mark.asyncio
async def test_run_async_workflow_full_path(engine: WPTGenEngine,
                                            mocker: MockerFixture) -> None:  # pylint: disable=line-too-long
    """Full asynchronous workflow orchestration, ensuring each phase is called."""  # pylint: disable=line-too-long
...
async def test_run_async_workflow_phase_failures(engine: WPTGenEngine,
    ...
    with pytest.raises(WorkflowError,
                       match='Phase 2: Requirements Extraction failed.'):
        await engine._run_async_workflow('feat-id')
...
async def test_run_async_workflow_suggestions_only(
        engine: WPTGenEngine, mocker: MockerFixture) -> None:  # pylint: disable=line-too-long
    """Verifies that the workflow short-circuits to provide_coverage_report when config.suggestions_only is True."""  # pylint: disable=line-too-long
    ...
    mock_provide = mocker.patch('wptgen.engine.provide_coverage_report',
                                return_value=None)
    mock_gen = mocker.patch('wptgen.engine.run_test_generation',
                            return_value=[])
...
async def test_run_async_workflow_detailed_requirements(
        engine: WPTGenEngine, mocker: MockerFixture) -> None:  # pylint: disable=line-too-long
    """Verifies that run_requirements_extraction_iterative is called when config.detailed_requirements is True."""  # pylint: disable=line-too-long
...
def test_engine_init(engine: WPTGenEngine, mock_config: Config) -> None:  # pylint: disable=line-too-long
    """Verifies that the engine initializes correctly with the given configuration."""  # pylint: disable=line-too-long
    assert engine.config == mock_config
    assert engine.llm is not None
    assert engine.jinja_env is not None


def test_engine_load_resume_state_invalid_json(mocker: MockerFixture,
                                               tmp_path: Path,
...
@pytest.mark.asyncio
async def test_engine_single_prompt_requirements(mocker: MockerFixture,
                                                 mock_config: Config) -> None:
...
@pytest.mark.asyncio
async def test_engine_skip_phase_4(mocker: MockerFixture,
                                   mock_config: Config) -> None:
    ui_mock = MagicMock()
    mocker.patch('wptgen.engine.get_llm_client')
    engine = WPTGenEngine(mock_config, ui_mock)
...
def test_engine_load_resume_state_missing(mocker: MockerFixture, tmp_path: Path,
                                          mock_config: Config) -> None:
    ui_mock = MagicMock()
    mocker.patch('wptgen.engine.get_llm_client')
    engine = WPTGenEngine(mock_config, ui_mock)
...
@pytest.mark.asyncio
async def test_engine_resume_workflow_success(mocker: MockerFixture,
                                              mock_config: Config) -> None:
...
def test_engine_load_resume_state_success(mocker: MockerFixture, tmp_path: Path,
                                          mock_config: Config) -> None:
    ui_mock = MagicMock()
    mocker.patch('wptgen.engine.get_llm_client')
    engine = WPTGenEngine(mock_config, ui_mock)
    resume_file = tmp_path / 'mock_feature_resume.json'
    resume_file.write_text(  # pylint: disable=line-too-long
...
def test_engine_hydrate_context(mocker: MockerFixture, tmp_path: Path,
                                mock_config: Config) -> None:
    ui_mock = MagicMock()
    mocker.patch('wptgen.engine.get_llm_client')
    ...
    (Path(mock_config.state_dir) /
     'requirements.json').write_text('{"requirements_xml": "<test-reqs/>"}')
    (Path(mock_config.state_dir) /
     'blueprints.json').write_text('{"audit_response": "<test-audit/>"}')
...
@pytest.mark.asyncio
async def test_engine_resume_from_generation(mocker: MockerFixture,
                                             mock_config: Config,
                                             tmp_path: Path) -> None:
    ui_mock = MagicMock()
    mocker.patch('wptgen.engine.get_llm_client')
...
async def test_run_async_workflow_brief_suggestions(
        engine: WPTGenEngine, mocker: MockerFixture) -> None:

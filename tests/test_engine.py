...
async def test_phase_test_generation_filename_sanitization(engine, mocker):
  ...
  # "Test: My Cool Feature!" -> "test_generated__test__my_cool_feature_.html"
...
async def test_phase_test_generation_tag_fallbacks(engine, mocker):
  ...
  # Based on engine.py: raw_title = self._extract_xml_tag(suggestion_xml, 'title') or 'file'
  mock_gen_save.assert_called_once()
...
async def test_phase_test_generation_duplicate_titles(engine, mocker):
  """Tests behavior when multiple suggestions have identical titles (should overwrite)."""
  context = {"metadata": MagicMock(name="Feat", description="Desc")}
  ...
  # Both should be "generated", but they will target the same filename
...
async def test_phase_test_generation_unicode_stability(engine, mocker):
  ...
  # Regex replaces non-alphanumeric with _, lowercases the rest
  # "Test 🚀 & 中文" -> "test____&___" (depending on regex)
  # Actual expected: test_______html (multiple underscores for non-matches)
  mock_gen_save.assert_called_once()

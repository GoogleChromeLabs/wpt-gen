...
def test_get_llm_client_gemini(
    ...
    """Test that the factory returns a GeminiClient when configured for
    Gemini.
    """
...
def test_gemini_client_gcp_adc(mocker: MockerFixture) -> None:
    """Test that GeminiClient initializes with vertexai=True when using ADC."""
    mock_client_class = mocker.patch("wptgen.llm.genai.Client")
    client = GeminiClient.from_adc(
    ...
    mock_client_class.assert_called_once_with(
        ...
        project="test-project",
        location="us-central1",
        http_options=_build_http_options(600),
    )
...
def test_get_llm_client_openai(
    ...
    """Test that the factory returns an OpenAIClient when configured for
    OpenAI.
    """
    # Prevent the actual SDK from initializing
...
def test_gemini_count_tokens_no_count(
    ...
    """Test that GeminiClient raises ValueError when API returns no token
    count.
    """
    mocker.patch("time.sleep")  # Bypass retry backoff delay
    mock_client_class = mocker.patch("wptgen.llm.genai.Client")
...
def test_gemini_generate_content_model_override(
    ...
    """Test that GeminiClient respects the model override in
    generate_content.
    """
    mock_client_class = mocker.patch("wptgen.llm.genai.Client")
    mock_instance = mock_client_class.return_value

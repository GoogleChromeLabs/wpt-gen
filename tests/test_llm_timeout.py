...
def test_gemini_client_timeout_passed(mocker: MockerFixture,
...
def test_openai_client_timeout_passed(mocker: MockerFixture,
                                      base_config: Config) -> None:
...
def test_gemini_timeout_handling(mocker: MockerFixture,
                                 base_config: Config) -> None:
    ...
    with pytest.raises(LLMTimeoutError,
                       match='Gemini API request timed out after 600s'):
        client.generate_content('test prompt')


def test_openai_timeout_handling(mocker: MockerFixture,
                                 base_config: Config) -> None:
    ...
    with pytest.raises(LLMTimeoutError,
                       match='OpenAI API request timed out after 600s'):
        client.generate_content('test prompt')


def test_get_llm_client_passes_timeout(mocker: MockerFixture,
                                       base_config: Config) -> None:  # pylint: disable=line-too-long

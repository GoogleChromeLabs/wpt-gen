...
def test_get_recent_test_files_success(
    ...
    # Note: the test simulates duplicate lines (different commits modifying
    # the same file)
    # and files that don't match the required
    # extension.
    mock_result.stdout = 'dir/test1.html\ndir/test1.html\ndir/ignore.js\ndir/test2.html\ndir/test3.html\n'  # pylint: disable=line-too-long
    mock_subprocess_run.return_value = mock_result

    mocker.patch(
        'wptgen.utils.Path.read_text',
        side_effect=['content 1', 'content 2', 'content 3'],
    )
    ...
    args, kwargs = mock_subprocess_run.call_args  # pylint: disable=unused-variable
    assert args[0][:5] == [
        'git',
        'log',
        '--name-only',
...
def test_get_recent_test_files_token_limit(
    ...
    mocker.patch(
        'wptgen.utils.Path.read_text',
        side_effect=[large_content, small_content],
    )
...
def test_get_recent_test_files_custom_token_counter(
    ...
    files = get_recent_test_files('dir',
                                  '.html',
                                  limit=2,
                                  max_tokens=20,
...
def test_get_recent_test_files_git_failure(mock_subprocess_run: MagicMock,
                                           mock_path_methods: None) -> None:
